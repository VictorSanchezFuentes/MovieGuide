...
export default function ImageField(props: imageFieldProps){
    ...
    const handleOnChange = (eventsArgs: ChangeEvent<HTMLInputElement>) => {
        if (eventsArgs.currentTarget.files){
            ...
            if(file){
                ...
                //if the user selects the file but then selects cancel
                setImageBase64("");
            }
        }
    }
    ...
    const toBase64 = (file : File) => {
        return new Promise<string>((resolve, reject) => {
            ...
            //conver into base64 representation
            reader.onerror = (error) => reject(error);
        })
    }//we need to convert it into its base64
    //representation to show it on page, so
